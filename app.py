        ...
        with ui.tab_panels(right_tabs, value=right_oracle_tab).classes("w-full"):
            ...
            with ui.tab_panel(right_node_info_tab).classes("p-2"):
                ...
                _CONTROL_RELS = {
                    ...
                    "DCSync", "DumpSMSAPassword", "DelegatedEnrollmentAgent",
                ...
                def _update_selected_node_panel(node_id: str) -> None:
                    ...
                    _fill_section(admin_container, sd["local_admin"], fmt=lambda i: i[1])
                    section_admin.text = f"Local Admin Privileges ({len(sd['local_admin'])})"
                    section_admin.set_visibility(bool(sd["local_admin"]))

                    _fill_section(exec_container, sd["exec_privs"], fmt=lambda i: f"{i[1]}  [{i[2]}]")
                    section_exec.text = f"Execution Privileges ({len(sd['exec_privs'])})"
                    section_exec.set_visibility(bool(sd["exec_privs"]))

                    _fill_section(inbound_container, sd["inbound"], fmt=lambda i: f"{i[1]}  [{i[2]}]")
                    section_inbound.text = f"Inbound Object Control ({len(sd['inbound'])})"
                    section_inbound.set_visibility(bool(sd["inbound"]))

                    _fill_section(outbound_container, sd["outbound"], fmt=lambda i: f"{i[1]}  [{i[2]}]")
                    section_outbound.text = f"Outbound Object Control ({len(sd['outbound'])})"
    ...
    with ui.column().classes("w-full p-6 gap-4"):
        with ui.expansion("Control Panel", icon="tune").props("default-opened")\
            ...
            with ui.card().classes("w-full bg-transparent border-0 shadow-none"):
                ...
                def _dismiss_path_suggestions(kind: str, hint_label: Any, container: Any) -> None:
                    container.clear()
                        ...
                        for node_id, label in candidates:
                            ...
                            def _fill_source(entry: str = label) -> None:
                                if path_source_input is not None:
                                    ...
                                    path_source_input.update()
                                _dismiss_path_suggestions("Source", hint_label, container)

                            def _fill_target(entry: str = label) -> None:
                                if path_target_input is not None:
                                    path_target_input.value = entry
                                    path_target_input.update()
                                _dismiss_path_suggestions("Target", hint_label, container)
                ...
                def _dismiss_owned_suggestions() -> None:
